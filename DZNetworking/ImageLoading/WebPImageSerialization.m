...
+ (UIImage *)imageWithWebPAtPath:(NSString *)filePath {
    ...
    if (isRetina) {
        ...
        BOOL at2XExists = [[NSFileManager defaultManager] fileExistsAtPath: [NSString stringWithFormat:@"%@.%@", at2XName, ext]];
...
- (NSData *)dataWebPWithQuality:(CGFloat)quality {
...
- (BOOL)writeWebPToDocumentsWithFileName:(NSString *)filename quality:(CGFloat)quality {
    NSString *filePath = [NSHomeDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"Caches/%@.webp",filename]];
